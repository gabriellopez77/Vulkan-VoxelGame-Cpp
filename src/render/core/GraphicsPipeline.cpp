...
void rk::GraphicsPipeline::create(const PipelineSettings& settings) {
    if (m_usePushConstant) {
        m_pushConstantShaderStage = static_cast<ShaderStage>(settings.pushConstantRange->stageFlags);
    ...
    rasterizer.cullMode = static_cast<VkCullModeFlags>(settings.cullMode);
    ...
    colorBlendAttachment.blendEnable = settings.enableBlending;
    ...
    depthStencil.depthTestEnable = settings.enableDepthTest;
    ...
    pipelineLayoutInfo.pushConstantRangeCount = static_cast<u32>(m_usePushConstant);
...
void rk::GraphicsPipeline::bind(VkCommandBuffer command) {
    ...
    vkCmdBindDescriptorSets(command, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelineLayout, 0,
        static_cast<u32>(m_descriptorSets.size()), m_vkDescriptorSets[vulkanApp::getImageIndex()].data(),
        static_cast<u32>(m_dynamicUboOffsets.size()), m_dynamicUboOffsets.data());
...
void rk::GraphicsPipeline::bindPushConstant(VkCommandBuffer command, u64 size, const void* data) const {
    assert(m_usePushConstant && "pipeline do not use push constant");

    vkCmdPushConstants(command, m_pipelineLayout, static_cast<VkShaderStageFlags>(m_pushConstantShaderStage), 0, size, data);
}

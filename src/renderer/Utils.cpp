...
namespace rk::utl {
    ...
    VkCommandBuffer beginSingleTimeCommand() {
        ...
        vkAllocateCommandBuffers(VulkanApp::get()->logicalDevice.get(), &allocInfo, &commandBuffer);
    ...
    void endSingleTimeCommand(VkCommandBuffer command) {
        ...
        vkFreeCommandBuffers(logicalDevice.get(), VulkanApp::get()->getTransferCommandPool(), 1, &command);
    }
    ...
    VkImageView createImageView(VkImage image, Formats format, u32 aspectFlag) {
        ...
        if (vkCreateImageView(VulkanApp::get()->logicalDevice.get(), &createInfo, nullptr, &imageView) != VK_SUCCESS)
